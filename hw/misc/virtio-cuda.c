...
#include "hw/virtio/virtio-vcuda.h"
#include <sys/mman.h>
...
static void virtio_vcuda_device_realize(DeviceState *dev, Error **errp)
{
    VirtIODevice *vdev = VIRTIO_DEVICE(dev);
    VirtIOVC *qcu = VIRTIO_VC(dev);
    //Error *err = NULL;

    //ptrace("GPU mem size=%"PRIu64"\n", qcu->conf.mem_size);
    virtio_init(vdev, "virtio-vcuda", VIRTIO_ID_VCUDA, sizeof(VirtIOVCConf));
    //qcu->vq  = virtio_add_queue(vdev, 1024, virtio_vcuda_cmd_handle);
}
static uint64_t virtio_vcuda_get_features(VirtIODevice *vdev, uint64_t features, Error **errp)
{
...
/*
   get the configure
ex: -device virtio-vcuda,size=2G,.....
DEFINE_PROP_SIZE(config name, device struce, element, default value)
 */
static Property virtio_vcuda_properties[] = 
{
    DEFINE_PROP_SIZE("size", VirtIOVC, conf.mem_size, 0),
    DEFINE_PROP_END_OF_LIST(),
};

static void virtio_vcuda_instance_init(Object *obj)
{
}

static void virtio_vcuda_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    VirtioDeviceClass *vdc = VIRTIO_DEVICE_CLASS(klass);

    dc->props = virtio_vcuda_properties;
    set_bit(DEVICE_CATEGORY_MISC, dc->categories);
    vdc->get_features = virtio_vcuda_get_features;
    vdc->realize = virtio_vcuda_device_realize;
}

static const TypeInfo virtio_vcuda_device_info = {
    .name = TYPE_VIRTIO_VC,
    .parent = TYPE_VIRTIO_DEVICE,
    .instance_size = sizeof(VirtIOVC),
    .instance_init = virtio_vcuda_instance_init,
    .class_init = virtio_vcuda_class_init,
};

static void virtio_vcuda_register_types(void)
{
    type_register_static(&virtio_vcuda_device_info);
}

type_init(virtio_vcuda_register_types)
